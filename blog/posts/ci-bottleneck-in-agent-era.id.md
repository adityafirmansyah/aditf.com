...
Lever pertama Linear itu nggak butuh ubah pipeline sama sekali: pindah dari GitHub Actions ke runner pihak ketiga yang lebih cepat bikin job rata-rata 34% lebih ngebut, sementara `tsc` type-check turun 52%. Nggak ada workflow yang perlu ditulis ulang, nggak ada tooling baru yang perlu dipelajari, cuma runner beda di bawah YAML yang sama.

Buat tim kecil atau budget UMKM, di volume job era-agent, runner hosted premium cepat jadi mahal, karena billing per-menit langsung numpuk pas jumlah job meledak, bukan mengecil. Self-hosted runner di hardware biasa ngubah hitungannya. Punya gue jalan di homelab yang malu-maluin kalau disebut di forum hardware, dan tetep menang dari sisi waktu sampai hasil pertama muncul: nggak ada antrean, nggak ada meter yang jalan, bukan soal hardware-nya yang cepat.

## Hitung berapa kali runner start, bukan berapa detik

Pelajaran kedua Linear: benerin critical path dulu sebelum micro-optimize apapun di dalemnya. Batasin fetch depth git aja bikin gate job paling lambat mereka turun dari 94 detik ke 20 detik. Batching tujuh check kecil jadi dua job hemat sekitar 87.000 runner-minute per bulan, 11,8% dari total spending CI mereka. Dua-duanya nggak nyentuh logic test sama sekali. Dua-duanya datang dari ngitung berapa job yang jalan, bukan berapa lama tiap job jalan.
...
Setiap job start punya overhead tetap (checkout, restore dependency, setup environment) sebelum dia jalanin satu assertion pun. Kalikan itu sama volume PR hasil agent, dan itu berhenti jadi angka kecil yang bisa diabaikan. Di pipeline kita bentuknya beda, lima job bukan tujuh kayak punya Linear, tapi fix-nya sama persis: gabungin jadi dua job motong rata-rata waktu gate PR hampir sepertiga, nggak ada satupun logic test yang disentuh.
...
Thread HN di bawah post Linear kebagi jadi dua kubu yang layak diketahui. Satu komentator, yieldcrv, bilang unit test secara umum udah jadi kosmetik yang nggembungin angka coverage, dan bikin poin yang layak direnungin: dia nggak lihat agent make test beda dari developer junior atau mid-level, karena manusia juga sebenarnya nggak rigorous-rigorous amat soal itu. Komentator lain, sz4kerto, bikin poin praktisi yang lebih tajam: review diam-diam bergeser dari review kode ke review test, karena di situlah sekarang letak klaim beneran soal correctness dari agent. Solomon Hykes, founder Docker, bilang build dan test harus dijadwalin sebagai satu sistem, bukan ditempel-tempel, nganggep verifikasi sebagai infrastruktur, bukan tempelan belakangan.

Setelah jalanin fleet ini tiap hari, pandangan gue simpel: gate ketat di money path, sekelompok kecil flow yang beneran bikin bisnis rusak kalau sampai gagal, dan biarin semua yang lain murah dan cepat. Test hasil agent yang hijau bukan berarti hal yang dia test itu beneran nyata. Agent bakal terus buka PR di kecepatan mereka sendiri, mau pipeline kamu siap atau enggak. Punya kita nggak siap, selama sembilan hari berturut-turut, sampai kita mulai nganggep runner starts dan kebijakan isolasi sebagai keputusan engineering, bukan tempelan di belakang checkmark hijau.
